...
use GenComm\Parser\GenLog\Transaction\Calculation;
use GenComm\GenLog;
use GenComm\Service\Http\Response\Response;
use GenComm\Service\Http\Webservice;
use GenComm\Enum\Status;
use GenComm\Parser\GenLog\Transaction\Autocomplete;
use GenComm\Parser\GenLog\Transaction\Batch;

class GenLogTest extends TestCase
{
    public function testItShouldCreateCalculationAndReturnSuccess()
    {
        ...
        $stubGenLog = $this->getMockBuilder(GenLog::class)
            ->setConstructorArgs(["fake-document", "fake-apikey", "fake-signature", Environment::SANDBOX])
            ->setMethods(['getWebservice'])
            ->getMock();
        $stubGenLog->expects($this->once())
            ->method('getWebservice')
            ->willReturn($stubWebservice);

        $calculation = $stubGenLog->calculation();

        $response = $stubGenLog->createCalculation($calculation);
    ...
    public function testItShouldCreateCalculationAndReturnException()
    {
        ...
        $stubGenLog->createCalculation($calculation);
    }
    ...
    public function testItShouldAutocompleteAndReturnSuccess()
    {
        ...
        $response = $stubGenLog->autocomplete("01415001", true);
    ...
    public function testItShouldGenerateBatchAndReturnSuccess()
    {
        ...
        $batch = $stubGenLog->batch();

        $response = $stubGenLog->generateBatch($batch);
